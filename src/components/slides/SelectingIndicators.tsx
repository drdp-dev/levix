  ...
  return (
    <div className={`w-full h-full flex flex-col p-8 relative overflow-hidden bg-white transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="flex-none mb-4 flex items-center justify-between z-10">
        <div>
            <h2 className={`text-4xl font-black text-olive-900 mb-2 transition-all duration-1000 transform ${isActive ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0'}`}>
            Step 3: Selecting Indicators
            </h2>
            <div className={`h-1.5 w-32 bg-olive-300 rounded-full transition-all duration-1000 delay-200 transform ${isActive ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0'}`}></div>
        </div>
      </div>
      ...
      <div className={`flex-none mt-4 bg-white border-2 border-red-100 rounded-3xl p-4 flex items-center justify-center gap-6 shadow-md transition-all duration-1000 delay-[1200ms] transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'} hover:border-red-300`}>
        ...
        <p className="text-2xl text-olive-900">
