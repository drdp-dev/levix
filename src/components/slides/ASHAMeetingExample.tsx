    ...
    <div className={`w-full h-full flex flex-col p-10 relative overflow-hidden bg-white transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="flex-none mb-10">
         <div className="flex items-center gap-4">
            <div className="p-3 bg-olive-100 rounded-2xl">
                 <Users className="w-10 h-10 text-olive-700" />
            </div>
            <div>
                ...
                <h3 className={`text-2xl text-olive-600 font-medium transition-all duration-1000 delay-200 transform ${isActive ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'}`}>
      ...
      <div className="flex-1 flex items-center justify-between px-4 gap-8">
        ...
        <div className={`flex-1 flex flex-col p-8 bg-red-50 border-2 border-red-100 rounded-3xl shadow-sm h-full justify-center transition-all duration-700 delay-300 transform ${isActive ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            ...
            <p className="text-4xl text-olive-900 font-medium leading-snug">
        ...
        {/* Solution Card */}
        <div className={`flex-1 flex flex-col p-8 bg-forest-50 border-2 border-forest-100 rounded-3xl shadow-sm h-full justify-center transition-all duration-700 delay-700 transform ${isActive ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
             <div className="flex items-center gap-4 mb-8">
                <div className="p-4 bg-white rounded-full text-forest-600 shadow-sm">
                    <CalendarCheck className="w-12 h-12" strokeWidth={1.5} />
                </div>
            </div>
            <h4 className="text-xl font-bold text-forest-800 uppercase tracking-widest mb-6">Programmatic Response</h4>
            <ul className="space-y-6">
                <li className="flex items-center gap-4 text-3xl text-olive-900 font-bold">
                    <div className="w-4 h-4 bg-forest-500 rounded-full flex-shrink-0"></div> Reschedule meetings
                </li>
                <li className="flex items-center gap-4 text-3xl text-olive-900 font-bold">
                    <div className="w-4 h-4 bg-forest-500 rounded-full flex-shrink-0"></div> Provide flexible timing
                </li>
            </ul>
        </div>
      ...
      {/* Footer Loop Concept */}
      <div className={`flex-none mt-10 bg-cornsilk-50 border-l-[12px] border-cornsilk-500 rounded-r-full p-6 flex items-center justify-center gap-6 shadow-md transition-all duration-1000 delay-1000 transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
         <Repeat className="w-10 h-10 text-cornsilk-700" strokeWidth={3} />
         <p className="text-3xl font-bold text-olive-900">
            Feedback closes the <span className="text-cornsilk-700 underline decoration-4 underline-offset-4">evaluation loop</span>.
         </p>
      </div>
