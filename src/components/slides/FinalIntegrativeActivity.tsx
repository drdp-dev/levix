...
const FinalIntegrativeActivity: React.FC<SlideProps> = ({ isActive }) => {
  ...
  const tasks = [
    ...
    { id: "task-5", text: "District action plan", icon: Megaphone, color: "text-lime-700", bg: "bg-lime-100", border: "border-lime-200" },
  ];
  ...
  const getThemeStyles = (theme: string) => {
      switch(theme) {
          ...
          case 'olive': return { bg: 'bg-lime-100', text: 'text-lime-900', icon: 'text-lime-700' };
    ...
    <div className={`w-full h-full flex flex-col p-8 relative overflow-hidden bg-white transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="flex-none mb-6 flex justify-between items-start relative z-10">
        <div className="flex-1 pr-6 pt-2">
            <div className="inline-flex items-center gap-2 px-5 py-2 bg-lime-200 rounded-full text-lime-900 font-bold uppercase tracking-widest text-sm mb-3">
                Final Activity
            </div>
            <h2 className="text-6xl font-bold text-lime-950 leading-tight">
              Design a Realistic<br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-lime-700 to-green-700">Evaluation Plan</span>
            </h2>
            <div className="flex items-center gap-2 mt-2 text-lime-1000 font-bold text-sm uppercase tracking-wider animate-pulse">
            ...
            <div className="flex items-center justify-between gap-8">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div className="text-4xl font-black text-lime-950">₹15 Lakh</div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div className="text-4xl font-black text-lime-950">12 Months</div>
                    </div>
                </div>
            </div>
            <div className="text-2xl font-bold text-lime-800 bg-lime-100 p-4 rounded-xl text-center border border-lime-300 shadow-inner">
      ...
      <div className="flex-1 grid grid-cols-3 grid-rows-2 gap-6 mb-6 relative z-10 min-h-0">
        {tasks.map((task, index) => (
            ...
            >
                ...
                <p className="text-3xl font-bold text-gray-800 leading-tight group-hover:text-lime-950 transition-colors relative z-10">
                    {task.text}
                </p>
                <div className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
                    <MousePointerClick className="w-8 h-8 text-lime-400" />
                </div>
            </div>
        ))}
      </div>
      ...
      >
         <div className="bg-gradient-to-r from-lime-950 to-green-950 rounded-[2.5rem] p-10 shadow-2xl flex items-center justify-between text-white relative overflow-hidden border-4 border-white/20 group-hover:scale-[1.01] transition-transform">
             {/* Decorative pattern */}
             ...
             <div className="flex items-center gap-10 relative z-10 w-full">
                ...
                <div className="flex-1">
                    <h4 className="text-lime-400 font-bold uppercase tracking-widest text-xl mb-3 flex items-center gap-3">
        ...
        <div className="absolute inset-0 z-[60] flex items-center justify-center backdrop-blur-md p-10 animate-in fade-in duration-300" onClick={() => setSelectedItem(null)}>
            ...
            >
                ...
                <div className="flex flex-col items-center text-center">
                    ...
                    <div className="w-full space-y-6 text-left bg-gray-50 p-8 rounded-3xl border border-gray-100">
                        {selectedItem.lines.map((line: string, i: number) => (
                            <div key={i} className="flex items-start gap-4">
                                ...
                                <p className="text-3xl font-medium text-lime-950 leading-snug">
                                    {line.includes('**') ? (
                                        <>
                                            {line.split('**')[0]}
                    ...
                    <div className="mt-8 flex items-center gap-3 text-lime-1000 font-bold uppercase tracking-widest text-sm">
