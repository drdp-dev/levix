  ...
  return (
    <div className={`w-full h-full flex flex-col p-10 relative overflow-hidden bg-white transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="flex-none mb-10 text-center relative z-10">
            <h2 className={`text-6xl font-bold text-olive-900 mb-2 transition-all duration-1000 transform ${isActive ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0'}`}>
                The Purpose of Evaluation
            </h2>
            <div className={`h-1.5 w-32 bg-olive-200 mx-auto rounded-full mb-4 transition-all duration-1000 delay-200 transform ${isActive ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0'}`}></div>
            <p className={`text-3xl text-olive-600 font-medium transition-all duration-1000 delay-300 transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        ...
        <div className="flex-1 grid grid-cols-3 gap-8 px-4 mb-4 relative z-10">
            {pillars.map((item, index) => (
                ...
                >
                    ...
                    <p className="text-3xl font-bold text-olive-900 leading-tight mb-6 flex-1 flex items-center">
                        "{item.question}"
                    </p>

                    <div className="w-full pt-6 border-t border-gray-200/50">
                        <span className="text-xl text-olive-600 font-medium flex items-center justify-center gap-2">
