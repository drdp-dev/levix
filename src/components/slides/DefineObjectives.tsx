  ...
  return (
    <div className={`w-full h-full flex flex-col p-10 relative overflow-hidden bg-white transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="flex-none mb-6 text-center">
        <h2 className={`text-5xl font-bold text-olive-900 mb-3 transition-all duration-1000 transform ${isActive ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0'}`}>
      ...
      <div className="flex-1 flex items-stretch justify-center gap-4 px-4 pb-4">
        {smart.map((item, index) => (
          ...
          >
            ...
            <span className="text-2xl font-bold text-olive-800">{item.word}</span>
          </div>
        ))}
      </div>
