...
const ThankYou: React.FC<SlideProps> = ({ isActive }) => {
  ...
  return (
    <div className={`w-full h-full flex flex-col p-12 relative bg-white transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
        {/* Background */}
        <div className="absolute top-0 left-0 w-3/4 h-3/4 bg-green-100 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 opacity-60 pointer-events-none animate-drift"></div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative z-10">
            ...
            <div className={`text-center mb-12 transition-all duration-1000 transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'} animate-float`}>
                <div className="inline-block p-6 bg-green-200 rounded-full mb-6 text-green-900 shadow-sm hover:shadow-lg transition-shadow duration-300">
                     <HeartHandshake className="w-20 h-20 animate-pulse" strokeWidth={1.5} />
                </div>
                <h1 className="text-7xl font-bold text-green-950 tracking-tight">Thank You</h1>
            </div>
            ...
            <div className={`w-full max-w-4xl bg-white/80 backdrop-blur border-2 border-green-200 rounded-3xl p-8 shadow-sm transition-all duration-1000 delay-700 transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'} hover:shadow-xl hover:border-green-300 transition-all`}>
                ...
                <ul className="space-y-4">
                    {references.map((ref, idx) => (
                        <li key={idx} className="flex items-start gap-4 text-lg text-green-800 hover:text-green-950 hover:translate-x-2 transition-all duration-300 cursor-default">
