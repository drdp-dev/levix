    ...
    <div className={`w-full h-full flex flex-col items-center justify-center relative bg-white transition-opacity duration-1000 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-6xl px-4">
        ...
        <div className="space-y-2">
            <h2 className={`text-4xl font-medium text-slate-500 uppercase tracking-[0.2em] transition-all duration-1000 delay-300 transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                Evaluation of
            </h2>
            
            <h1 className="flex flex-col gap-2">
                <span className={`text-8xl font-black text-slate-900 tracking-tight leading-none transition-all duration-1000 delay-500 transform ${isActive ? 'translate-x-0 opacity-100' : '-translate-x-20 opacity-0'}`}>
                    Health <span className="text-transparent bg-clip-text bg-gradient-to-r from-lime-600 to-emerald-600">Promotion</span>
                </span>
                <span className={`text-6xl font-bold text-slate-300 transition-all duration-1000 delay-700 transform ${isActive ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
                    &
                </span>
                <span className={`text-8xl font-black text-slate-900 tracking-tight leading-none transition-all duration-1000 delay-900 transform ${isActive ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'}`}>
                    Health <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Education</span>
                </span>
            </h1>
            
            <h2 className={`text-4xl font-light text-slate-400 uppercase tracking-[0.2em] mt-4 transition-all duration-1000 delay-1000 transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                Programs
            </h2>
        </div>

        {/* Presenter Badge */}
        <div className={`mt-16 bg-white/80 backdrop-blur-sm border border-slate-200 rounded-full pl-3 pr-8 py-3 shadow-lg flex items-center gap-4 transition-all duration-1000 delay-[1200ms] transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'} hover:scale-105 hover:shadow-xl`}>
            <div className="w-16 h-16 bg-gradient-to-br from-lime-700 to-emerald-700 rounded-full flex items-center justify-center text-white shadow-inner">
                <Stethoscope className="w-8 h-8" />
            </div>
            <div className="text-left">
                <p className="text-sm font-bold text-slate-400 uppercase tracking-wider">Presented By</p>
                <p className="text-2xl font-bold text-slate-900">Dr. Devaprasad Reddy</p>
            </div>
        </div>
      ...
      <div className={`absolute bottom-10 flex flex-col items-center gap-2 transition-all duration-1000 delay-[1500ms] ${isActive ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
         <span className="text-sm font-bold text-slate-400 uppercase tracking-widest animate-pulse">Start Presentation</span>
