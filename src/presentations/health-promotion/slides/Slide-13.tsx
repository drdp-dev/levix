...
const DesignCritiqueActivity: React.FC<SlideProps> = ({ isActive }) => {
  return (
    <div className={`w-full h-full flex flex-col p-10 relative bg-white transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
      ...
      {/* Background Decor */}
      <div className="absolute bottom-0 right-0 w-2/3 h-2/3 bg-yellow-100 rounded-full blur-3xl translate-y-1/2 translate-x-1/2 opacity-60 pointer-events-none"></div>
